...
def guard_diagnostic_agent_tool(diagnostic_script: str):
    """Validate diagnostic scripts provided by diagnostic agents.
    
    Args:
        diagnostic_script: The diagnostic script that is generated by the diagnostic agents
        
    Returns:
        Response back with one of "safe to excute" or "not safe to execute"
    """
    try:
        logger.info(f"Calling guard diagnostic  Agent....")
        # print("guard agent",user_input)
        prompt = f"""
            Your primary responsibility is to validate diagnostic scripts provided by diagnostic agents, ensuring they are safe and comply with system integrity guidelines. 
            Follow the instructions below in a clear and structured manner:
            ...
            4. Approval for Execution:
                ...
                - STRICTLY respond back with one of "safe to excute" if it is approved for execution
            ...
            Diagnostic script is : {diagnostic_script}
    """
