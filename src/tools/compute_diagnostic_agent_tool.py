...
def compute_diagnostic_agent_tool(user_input: str):
    ...
    Args:
        user_input (str): The user's description of the compute issue or event data from coordination agent.
    ...
    """
    try:
        logger.info(f"Calling compute diagnostic Agent....")

        prompt = f"""
        You are the Compute Diagnostic Agent. Your role is to analyze reported compute problems, generate a diagnostic bash script, and provide it in a structured JSON format.
        ...
        - Start the script with a header that describes its purpose, e.g.:
            ```bash
            # Compute Diagnostic Script for {user_input} on {user_input}
            ```
