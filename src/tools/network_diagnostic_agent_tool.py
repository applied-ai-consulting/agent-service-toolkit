...
def network_diagnostic_agent_tool(user_input: str):
    """
    Your primary role is to analyze network reported problems, generate precise and actionable diagnostic scripts, and evaluate available data to determine the root cause.

    Args:
        user_input (str): The user's description of the network issue or event data from coordination agent.
    ...
    """
    try:
        logger.info(f"Calling network diagnostic Agent....")

        prompt = f"""
        You are the Network Diagnostic Agent. Your role is to analyze reported network problems, generate a diagnostic bash script, and provide it in a structured JSON format.
            ...
            - Start the script with a header that describes its purpose, e.g.:
                ```bash
                # Network Diagnostic Script for {user_input} on {user_input}
                ```
            ...
            - `hostname`
            - Commands to collect relevant network diagnostics (e.g., `df -h`, `lsblk`, etc.).
